...
- `[ ]` 移除 `MaxWayMarkSlots = 30` 作为格式硬上限。
- `[ ]` 改用动态槽位数量：`slotCount = (data.Length - 16 - 4) / 104`。
...
- `[ ]` `KnownCurrentWayMarkSlots = 30` 仅用于 UI 提示、日志或测试说明，不用于拒绝解析。
- `[ ]` 如果未来游戏增加槽位，加载和保存应能保留全部槽位。
- `[ ]` UI 若暂不支持编辑超过 30 个槽位，应限制编辑行为而不是拒绝文件加载。
...
- `[~]` data 不足 16 字节时报错。
- `[ ]` data 长度不足 20 字节时报错。
- `[ ]` FMARKER 长度为 `16 + 104 * 30 + 4` 时通过。
- `[ ]` FMARKER 长度为 `16 + 104 * 5 + 4` 时通过，但测试名称不要写成旧版兼容。
- `[ ]` 合成 `16 + 104 * 31 + 4` 或更多槽位时应通过，证明没有 30 槽硬上限。
...
- `[ ]` `SectionFMARKER.ValidateForSave()` 不再因为 WayMark 数量超过 30 而失败。
